        ...
        orderData.map((item,index) =>(
          <div key={index} className="py-4 border-b border-brand-blue-800 text-brand-blue-300 flex flex-col md:flex-row md:items-center md:justify-between gap-4">
            <div className="flex items-start gap-6 text-sm">
                <img className="w-16 sm:w-20 border border-gray-200 rounded-sm" src={item.image[0]} alt="item_image" />
            ...
            <div className="md:w-1/2 flex justify-between">
              <div className="flex items-center gap-2">
                  ...
                  <p className="text-sm md:text-base text-brand-blue-50">{item.status}</p>
              </div>
              <button onClick={loadOrderData} className="border border-brand-blue-600 px-4 py-2 text-sm font-medium rounded-sm bg-black text-brand-blue-50 hover:bg-brand-blue-900 transition-colors">Track Order</button>
            </div>
          </div>
        ))
      )}
